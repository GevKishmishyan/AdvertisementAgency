...
public class AdMain implements Commands {
    ...
    private static void importUsersFromXlsx() {
        ...
        for (User user : users) {
            try {
                if (user.equals(STORAGE.getUserByPhoneNumber(user.getPhoneNumber()))) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (file.exists() && file.isDirectory()) {
                    XLSXUtil.writeAds(path, STORAGE.getAds());
                } else {
    ...
    private static void importAdsFromXlsx() {
        ...
        if (file.exists() && file.isFile()) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    List<Ad> ads = XLSXUtil.readAds(xlsxPath);
                    for (Ad ad : ads) {
                        try {
                            if (STORAGE.getAdByTitleStr(ad.getTitle()) == null) {
                                ad.setAuthor(STORAGE.getUserByPhoneNumber(ad.getAuthor().getPhoneNumber()));
                                STORAGE.add(ad);
                            } else {
